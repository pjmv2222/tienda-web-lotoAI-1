...
datos = pd.read_csv('C:\\Users\\Pedro\\Desktop\\LotoIA\\Lototurf\\DataFrame_Lototurf.csv', encoding='utf-8')
...
X = datos[['Num1', 'Num2', 'Num3', 'Num4', 'Num5', 'Num6', 'Horse']]
y = datos[['Num1', 'Num2', 'Num3', 'Num4', 'Num5', 'Num6', 'Horse']]  # Asegurarse de que 'Horse' es el nombre correcto de la columna
...
# Guardar el modelo
model.save('C:/Users/Pedro/Desktop/LotoIA/Lototurf/modelo_Lototurf.h5')

# Cargar el modelo
modelo = load_model('C:/Users/Pedro/Desktop/LotoIA/Lototurf/modelo_Lototurf.h5')
...
def ajustar_prediccion(prediccion):
    ...
    Horse, indice_Horse = np.unique(prediccion[6:], return_index=True)
    ...
    Horse_ordenados = Horse[np.argsort(indice_Horse)]
    
    # Combinar y retornar la predicción ajustada
    return np.concatenate((numeros_principales_ordenados, Horse_ordenados))
...
new_inputs = np.random.randint(1, 32, size=(100, 6))  # Para Num_1 a Num_6
new_stars = np.random.randint(1, 13, size=(100, 1))  # Para Horse
...
df = pd.DataFrame(new_predictions, columns=['Num1', 'Num2', 'Num3', 'Num4', 'Num5', 'Num6', 'Horse'])
...
def calcular_precision(predicciones, reales):
    ...
    for pred, real in zip(predicciones, reales):
        ...
        # Comparar Horse (últimos 2 valores)
        aciertos_Horse = len(set(pred[5:]) & set(real[5:]))
        total_aciertos_Horse += aciertos_Horse

    # Calcular métricas
    precision_media_numeros = total_aciertos_numeros / total_predicciones
    precision_media_Horse = total_aciertos_Horse / total_predicciones
    
    return precision_media_numeros, precision_media_Horse
...
precision_numeros, precision_Horse = calcular_precision(predicciones_ajustadas_int, y_test_int)

# Paso 3: Reportar los Resultados
print(f'Precisión media de números acertados por predicción: {precision_numeros}')
print(f'Precisión media de Horse acertados por predicción: {precision_Horse}')

# Guardar el DataFrame en un archivo CSV
df.to_csv('predicciones-Lototurf.csv', index=False)
...
datos.to_csv('C:\\Users\\Pedro\\Desktop\\LotoIA\\Lototurf\\Lototurf_Predicciones.csv', index=False, encoding='utf-8')