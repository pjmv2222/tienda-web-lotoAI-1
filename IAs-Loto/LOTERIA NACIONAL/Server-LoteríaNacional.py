...
def predict():
    if request.method == 'POST':
        ...
        # Generar predicción de un número de 5 dígitos para la Lotería Nacional
        numero_predicho = ''.join([str(random.randint(0, 9)) for _ in range(5)])
        
        # Crear la respuesta con la predicción
        response = {
            'status': 'Recibido', 
            'input': data['input'], 
            'prediction': {'numero': numero_predicho}
        }
