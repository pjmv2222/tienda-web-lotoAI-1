...
def ajustar_prediccion(prediccion):
    # Asegurar que los números principales (6 primeros números) no tengan duplicados
    numeros_principales = np.unique(prediccion[:6], return_index=True)
    # Asegurar que el reintegro (último número) no tenga duplicados
    Refund = np.unique(prediccion[6:], return_index=True)

    # Si hay duplicados, rellenar hasta tener la cantidad necesaria
    while len(numeros_principales[0]) < 6:
    ...
    while len(Refund[0]) < 1:
        nuevo_Refund = np.random.randint(1, 10)
        if nuevo_Refund not in Refund[0]:
            Refund[0] = np.append(Refund[0], nuevo_Refund)
            Refund[1] = np.append(Refund[1], -1)  # -1 como marcador de posición
