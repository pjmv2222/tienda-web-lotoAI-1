...
@Component({
  ...
  styleUrl: './elgordo-prediccion.component.css'
})
export class ElgordoPrediccionComponent implements OnInit, OnDestroy {
  isLoggedIn = false;
  ...
  private loadPredictionStatus() {
    this.userPredictionService.getPredictionStatus('elgordo').subscribe({
      next: (response) => {
        if (response.success) {
  ...
  async generatePrediction() {
    ...
    try {
      const canGenerate = await this.userPredictionService.canGeneratePrediction('elgordo').toPromise();
      if (!canGenerate) {
      ...
      this.showEmptyBalls = false;
      const response = await this.predictionService.generatePrediction('elgordo').toPromise();
      if (response && response.success && response.prediction) {
        await this.userPredictionService.createPrediction('elgordo', response.prediction).toPromise();
        this.predictionResults.push({
          numeros: response.prediction.numeros || [],
          clave: response.prediction.clave || null
        });
        this.updatePredictionDisplay();
      } else {
  ...
  private loadSavedPredictions(): void {
    try {
      const savedJson = localStorage.getItem('elgordo_predictions');
      if (savedJson) {
        ...
        if (hoursDiff < 24) {
          if (Array.isArray(savedData.predictions)) {
            this.predictionResults = savedData.predictions.map((pred: any) => ({
              ...
              clave: typeof pred.clave === 'number' ? pred.clave : null
            }));
            this.updateNumberFrequency();
          } else {
          ...
          localStorage.removeItem('elgordo_predictions');
        }
      }
    } catch {
      this.predictionResults = [];
  ...
  private loadPredictionsFromStorage() {
    const saved = localStorage.getItem('elgordoPredictions');
    if (saved) {
