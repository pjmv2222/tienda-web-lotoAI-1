...
@Component({
  ...
  styleUrl: './eurodreams-prediccion.component.css'
})
export class EurodreamsPrediccionComponent implements OnInit, OnDestroy {
  isLoggedIn = false;
  ...
  private loadPredictionStatus() {
    this.userPredictionService.getPredictionStatus('eurodreams').subscribe({
      next: (response) => {
        if (response.success) {
  ...
  private cargarBoteActual(): void {
    this.http.get('assets/botes.json').subscribe({
      next: (data: any) => {
        if (data && data.eurodreams) this.boteActual = data.eurodreams;
  ...
  async generatePrediction() {
    ...
    try {
      const canGenerate = await this.userPredictionService.canGeneratePrediction('eurodreams').toPromise();
      if (!canGenerate) {
      ...
      this.showEmptyBalls = false;
      const response = await this.predictionService.generatePrediction('eurodreams').toPromise();
      if (response && response.success && response.prediction) {
        await this.userPredictionService.createPrediction('eurodreams', response.prediction).toPromise();
        this.predictionResults.push({
          id: Date.now(),
          numeros: response.prediction.numeros || [],
          dream: response.prediction.dream || null,
          timestamp: new Date()
        });
        this.updatePredictionDisplay();
      } else {
  ...
  private savePredictionsToStorage(): void {
    try {
      ...
      localStorage.setItem('eurodreams_predictions', JSON.stringify(dataToSave));
    } catch {}
  }
  ...
  copyPredictionToClipboard(prediction: any): void {
    let textToCopy = `Predicción EuroDreams - ${this.formatTimestamp(prediction.timestamp)}\n`;
    textToCopy += `Números: ${prediction.numeros.join(', ')}`;
    
    if (prediction.dream !== null) {
      textToCopy += `\nDream: ${prediction.dream}`;
    }
