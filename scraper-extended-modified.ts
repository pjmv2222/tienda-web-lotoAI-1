            ...
            try {
                ...
                const gameConfigs = [
                    ...
                    },
                    {
                        ...
                        code: 'LNAC',
                        isSpecial: true,
                        isMultiple: true, // NUEVO: Indica múltiples sorteos
                ...
                gameConfigs.forEach(config => {
                    try {
                        // NUEVO: Manejo especial para Lotería Nacional con múltiples sorteos
                        if (config.isSpecial && config.isMultiple && config.game === 'loterianacional') {
                            ...
                            if (sorteoJueves) {
                                ...
                                if (premiosJueves.length > 0) {
                                    ...
                                    console.log(`✅ Lotería Nacional JUEVES: ${premiosJueves.length} premios, ${reintegrosJueves.length} reintegros`);
                                }
                            }
                            
                            // Procesar sorteo del SÁBADO (LNACS)
                            const sorteoSabado = document.querySelector(config.sabadoSelector || '');
                            if (sorteoSabado) {
                                const premiosSabado: string[] = [];
                                const reintegrosSabado: string[] = [];
                                
                                // Extraer premios del sábado
                                ...
                                if (premiosSabado.length > 0) {
                                    ...
                                    console.log(`✅ Lotería Nacional SÁBADO: ${premiosSabado.length} premios, ${reintegrosSabado.length} reintegros`);
                                }
                            }
                            
                            // Crear resultado con múltiples sorteos
                            if (sorteos.length > 0) {
                                const fechaMasReciente = sorteos[sorteos.length - 1].fecha; // Usar la fecha más reciente
                                const result: any = {
                                    game: config.game,
                                    sorteos: sorteos,
                ...
                // Función auxiliar para extraer fecha válida (SIN CAMBIOS)
